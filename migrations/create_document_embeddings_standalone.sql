-- Migration: Create document_embeddings table for vector storage (STANDALONE)
-- Description: Stores text chunks and their embeddings for RAG retrieval
-- Date: 2025-10-11
-- Note: This version works WITHOUT requiring bots/documents tables to exist first

-- Enable pgvector extension
CREATE EXTENSION IF NOT EXISTS vector;

-- Create document_embeddings table WITHOUT foreign key constraints
...
-- Note: This will initially fail if table is empty, but that's OK
DO $$
BEGIN
  IF NOT EXISTS (
...
DO $$
BEGIN
  RAISE NOTICE '✅ document_embeddings table created successfully';
  RAISE NOTICE '✅ Vector dimension: 3072 (text-embedding-3-large)';
  RAISE NOTICE '✅ match_document_embeddings() function created';
  RAISE NOTICE '⚠️  Note: Foreign key constraints will be added automatically by TypeORM when bots/documents tables are created';
END $$;
