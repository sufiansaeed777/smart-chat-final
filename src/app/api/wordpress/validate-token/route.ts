...
export async function POST(request: NextRequest) {
  try {
    ...
    if (bot.domain && domain) {
      // Normalize domains for comparison (remove www and protocol)
      const botDomain = bot.domain.replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/$/, '').toLowerCase();
      const requestDomain = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/$/, '').toLowerCase();

      // Check if domains match (exact or subdomain)
      const isValidDomain = requestDomain === botDomain ||
                           requestDomain.endsWith(`.${botDomain}`) ||
                           botDomain === 'localhost' || // Allow localhost for development
                           requestDomain.includes('localhost'); // Allow localhost with port

      if (!isValidDomain) {
        console.warn(`Domain validation failed: Expected ${bot.domain}, got ${domain}`);
        return NextResponse.json(
          {
            valid: false,
