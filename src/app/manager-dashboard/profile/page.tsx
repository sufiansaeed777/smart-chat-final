  ...
  const loadProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const profile = {
          firstName: data.profile?.firstName || data.profile?.name?.split(' ')[0] || session?.user?.name?.split(' ')[0] || '',
          lastName: data.profile?.lastName || data.profile?.name?.split(' ')[1] || session?.user?.name?.split(' ')[1] || '',
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        const response = await fetch('/api/manager/profile/avatar', {
          method: 'POST',
          body: formData
        });
